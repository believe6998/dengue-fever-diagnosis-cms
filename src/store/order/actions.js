...
export async function loadStatistical ({ commit, dispatch }, filter) {
  commit('fetchStatisticalBegin')
  try {
    cleanFilter(filter)
    const queryParams = new URLSearchParams({
      ...filter
    }).toString()
    const response = await httpClient.get(`${endPoint}/statistical?${queryParams}`)
    commit('fetchStatisticalSuccess', {
      data: response,
      filter: filter
    })
  } catch (error) {
    commit('fetchStatisticalError', error.response)
    return null
  }
}
